...
<c:if test="${type==1 }">
	<div class="btn-group">
		<button class="btn" onclick="window.location.href='/admin/model/addModifyModelPage.do?topicid=${topicid}&pageNum=${pageNum }&type=${type }&modelType=0'" title="新建模板">
		新建
		</button>
 	...
 	<c:if test="${type==1 }">
 		<c:forEach var="model" items="${modelList}" varStatus="modelStatus">
	 		<tr>
	 			...
	 			<td>
	 			<a target="blank" href="/admin/model/addModifyModelPage.do?modelName=${model.modelName }&topicid=${topicid}&pageNum=${pageNum }&type=${type }&modelType=${model.type}">
	 			<img src="/images/model.gif"/>&nbsp;&nbsp;${model.title}
	 			</a>&nbsp;&nbsp;
	 			<a onclick="modifyPushRecord('${pushRecord.indexid}')" href="#" title="修改属性" class="btn-small"><img src="/images/edit.gif"/></a></td>
	 			<td>${model.url }</td>
